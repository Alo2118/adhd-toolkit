// Active Tasks Management Functions - Temp file to be merged
// These functions will be added to index.html before </script>
  ...
  filtered.forEach(task=>{
    ...
    if(task.status==='completed')cardClass+=' completed';
      ...
      if(daysUntil<0){
        ...
        deadlineText='⚠️ Scaduto '+Math.abs(daysUntil)+' giorni fa';
      }else if(daysUntil===0){
        cardClass+=' near-deadline';
        deadlineClass='urgent';
        deadlineText='⏰ Scade oggi';
      }else if(daysUntil===1){
        cardClass+=' near-deadline';
        deadlineClass='soon';
        deadlineText='⏰ Scade domani';
      }else if(daysUntil<=state.notificationSettings.deadlineWarningDays){
        cardClass+=' near-deadline';
        deadlineClass='soon';
        deadlineText='⏰ Scade tra '+daysUntil+' giorni';
      }else{
        deadlineText='📅 Scade tra '+daysUntil+' giorni';
      }
    }
    ...
    h+='<div class="task-card-name">'+task.name+'</div>';
    ...
    if(daysSince>=state.notificationSettings.taskStuckDays&&task.status==='active'){
